...
- [x] Suite completa `run_idle_tests.gd`: 1124 checks, 0 failures | `benchmarks.gd`: 0 failures
      ...
      re-medido de novo em 2026-09-25 **nesta máquina**, depois da cópia de HD, com as mesmas nove contagens —
      idle 2257/0, RPC identity 10/0, e2e 0, backup 8/0, benchmarks 0, companion 100/0, security 47/0,
      refund CLI 12/0, estrutura 0 falhas em 278 arquivos — e a passada **commitada** em `6277671`, push feito)
...
- [x] S2: AH seed de bots feito e medido (`SuiteAHBots`: gated off/on, segunda rodada não duplica, buy path real consome, sem reseed após compra, `ReconcileDaily` 0); trava `SHAMBLETA_AH_BOTS` **desligada no beta** por decisão registrada. Residual honesto: nenhum check injeta falha no meio da transação do seed — a proteção contra duplicação em crash é rollback de `sql.Transaction` + skip por listing existente, argumento de desenho e não medição. Vai como dívida pós-beta (detalhe em `ROADMAP_COMERCIAL.md` S2)
