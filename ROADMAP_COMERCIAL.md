...
| `vip.1mo` | R$ 24,90/mês | QoL: offline cap 24h sem anúncio, settle ×1.2, tier 2 líquida 2×. Sem power direto |
| `pass.s1` / `pass.s1.deluxe` | R$ 24,90 / R$ 44,90 *(preço em vigor no código)* | Temporada 30d, regras congeladas, cosmético + chaves. Esta linha dizia R$ 19,90: desde que o SKU virou entregável os dois catálogos (`data/conf/paid_catalog.json`, `EconomyCatalog.gd`) cobram R$ 24,90, e registrei aqui o que está em vigor, não o que foi proposto. Baixar para R$ 19,90 é decisão do dono e mexe nos dois catálogos + `companion/server.py`, não só nesta linha |
...
- [x] **Regressão silenciosa da Fatia 1 caçada e corrigida**: consts movidos p/ `EconomyCatalog` deixaram de resolver quando acessados *via instância* — `economy.CONST` estoura "Invalid access to property or key" e o GDScript **aborta a função no meio**. 9 acessos em `tests/IdleTests.gd` (3 suítes paravam antes da metade: `SuiteEconomyShop`, `SuiteDailyShop`, `SuiteArena`) → trocados p/ `EconomyCatalog.*`; 1 acesso real de produção em `Server.WatchAd` (`Launcher.Economy.AD_AFK2X`) → o painel de AFK nunca era atualizado no 2× → corrigido. [2026-09-25: `afk2x` saiu — o placement hoje é `afkhoras` e compra hora em vez de dobrar loot.] Varredura programática do repositório inteiro: 0 restantes
...
- [x] **Fatia 8 do EconomyService (monetização de janela)**: `AdsCosmeticsService.gd` (237 linhas: Fase E rewarded ads — `_AdDayStart`/`AdViewsToday`/`_ValidAdToken`/`_AdAllowed`/`_RecordAdView`/`WatchAd`/`IsAfkAdArmed`/`ClaimAdChest`/`RerollDailyShopAd`/`ClaimAdBossKey`, com rate-limit por placement e token assinado — [2026-09-25: `IsAfkAdArmed` saiu junto do placement `afk2x`; entrou `AfkHoursEarned`, que compra hora] — e Fase D cosméticos/entitlements — `HasCosmetic`/`GrantCosmetic`/`GetCosmetics`/`Equip`/`Unequip`/`BuyCosmetic`, backfill de títulos de suporte, `_RebirthVitrine` e `EquippedTitleLabel`); `EconomyService.gd` 1757→1601 (−156). Verbatim conferido por diff programático (range 1036–1260); `_eco.` em 5 compartilhados (`settleMutex`, `_LedgerAppendLocked`, `_DailyRow`, `_DoReroll`, `GrantBossKey`); `CosmeticLabel` permaneceu **static** no wrapper e passou a chamar `AdsCosmeticsService.CosmeticLabel` via `class_name` (contexto estático não enxerga campo de instância — `Server.gd:538`/`WorldCommands.gd:609` chamam pela classe)
...
- **Suíte idle: 2356 checks, 0 failures, 0 SCRIPT ERROR** (1232 → 1304 com a suíte ampliada → 1329 ao destravar 25 checks que abortavam em silêncio, ver Fatia 5 → **1908** com a passada de beta: régua de retenção medida, K1, R3, G1/`SuiteSeasonBootstrap` e os guards de superfície → **2169** com a guarda de hotkeys `SuiteInputHotkeys` → **2184** com a decisão de boot das migrations `SQL.MigrationPlan` e as guards do filtro de export → **2220** com a segunda porta do checkout web, o inventário medido de painéis que nascem de `.new()` e os checks que sobraram de ver as mordaças mordendo → **2222** com o guard de i18n: toda `tr("literal")` de `sources/` tem que ter `pt_BR` no catálogo **compilado** → **2243** com os guards de boot web e a superfície de serviços → **2255** com a fiação do update do PWA → **2257** com o censo de duplicadas do `ui.csv` (chave repetida na primeira coluna sobrescreve a anterior no importador, e foi assim que `"Attack"` chegou ao pt-BR como verbo) (`/tmp/suite_beta_final28.log`, 9× `Gate §24-8 OK`) → **2327** com a hora comprada por anúncio e a vitrine honesta (passada de monetização, já commitada; `/tmp/all_precommit2.log`, 9× `Gate §24-8 OK`, `ALL_EXIT=0`) → **2356** com a passada de performance: `SuiteProgressUpsert` mede a **semântica** do save em lote que o novo probe de benchmark mede o custo — linha que não atualiza, chave que colide com outro personagem e chunk de 150 que engole o resto (`/tmp/shambleta-idle.log`) — e a régua completa fechou verde com a documentação já congelada: `ALL_EXIT=0`, 9× `Gate §24-8 OK`, preflight nos 6 harnesses, estrutura 0 falhas em 278 arquivos, RPC 10/0, e2e 0, backup 8/0, benchmarks 0, companion 170/0, security 47/0, refund 12/0 (`/tmp/shambleta-all-final2.log`). Rodada repetida e verde após **cada** uma das 12 fatias e após cada item do Bloco 0/1
...
- **Passada commitada e no remoto (2026-09-25):** `6277671` levou os 148 caminhos num commit só, `da2531c..6277671` em `origin/master`. Os nove gates foram re-medidos **nesta máquina** antes do commit, com as mesmas contagens (idle 2257/0 · rpc 10/0 · e2e 0 · backup 8/0 · benchmarks 0 · companion 100/0 · security 47/0 · refund 12/0 · estrutura 0/278 arquivos) · **Artefato Web re-produzido do zero aqui** (`scripts/export_web.sh`): first-load **36 MiB** gzip (engine 12 · pck 21 · shell 3) e boot num Chromium real em `== RESULT: 10 checks, 0 failures ==` — engine no console, `crossOriginIsolated`, `SharedArrayBuffer`, canvas 780×437, manifest `standalone`, exatamente um service worker no escopo raiz, zero request falho, zero erro de console. Continua **não medível** sem servidor: o que estes checks provam é o pacote, não o TLS de produção, o WSS contra o `game` real nem o `POST /checkout/preference` na origem do site (ver §4 do handoff). **Re-medido em 2026-09-26: first-load 35 MiB gzip** (`36.734.788` B, 18 arquivos; `.pck` 21 → 20 MiB) com o corte de `tests/*` e logo de imprensa do preset Web, boot revificado em Chromium em `== RESULT: 10 checks, 0 failures ==` — cadeia e armadilha do `#` em `export_presets.cfg` em `deploy/WEB_SLIM.md`
...
- **Benchmarks: 0 failures** (reconstruído em 2026-09-26; `/tmp/shambleta-bench-5.log`, reproduzido em `/tmp/shambleta-bench-4.log`) — settle **1 ms, 11 queries, 1 tx** (orçamento 500 ms) · catálogo **0 ms para 24 zonas** com falha explícita se zona sumir · **leaderboard 1 ms, 50 de 60 personagens**, agora com asserção de `EXPLAIN QUERY PLAN`: usa `idx_character_leaderboard`, ordem `power_score DESC` e **nada** de temp B-tree (é o guard da migration 047, e sem ele o índice podia sumir sem o gate perceber) · **browse do leilão 0 ms, 20 de 200 realces** em `id DESC` via `idx_auction_browse` · **progress save 1 ms, 3 queries, 1 tx para 150 entradas** (orçamento ≤ 12 queries, ≤ 1 tx) · **load probe 800 settles: p50 382 µs, p99 527 µs, max 427118 µs**, 2 hitches acima de 50 ms contra orçamento de 16, 0 erros, +0 objetos / +0 recursos · **o XP walk saiu do gate**, porque era um laço `totalXp += 10` que media zero — exatamente a crítica do §14 da auditoria independente. O que o probe agora mede é a **taxa** de hitch do checkpoint do WAL, não só o p99 (`PRAGMA wal_autocheckpoint=4000`). · **Backup-restore probe: 8 checks, 0 failures** (migration 47 = live 47, re-medido em `/tmp/shambleta-all-final2.log`; eram 46 até a 047) · **RPC identity: 10 checks, 0 failures** · **API cross-file: 0 failures** · **Companion: 170+47 checks, 0 failures** (os 70 novos do companion são a varredura de rótulo de cobrança nos dois catálogos) · **Refund CLI: 12 checks, 0 failures** · Os nove gates de `./scripts/test.sh all` (cinco headless Godot + um shell + três Python) passam pelo mesmo `scripts/ci_gate_log.sh`
- **Segfault de shutdown corrigido (S3 fechada)** — duas causas independentes, ambas
  pré-existentes no código commitado (nenhuma delas introduzida pelas fatias):
