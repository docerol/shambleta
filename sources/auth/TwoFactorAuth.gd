...
static func GenerateTOTP(secret: String, timestamp: int = -1) -> String:
    ...
    var counter: int = timestamp / TOTP_STEP_SECONDS
...
static func VerifyTOTP(secret: String, token: String, timestamp: int = -1) -> bool:
    if timestamp < 0:
        timestamp = int(Time.get_unix_time_from_system())
    var expected: String = GenerateTOTP(secret, timestamp)
    if expected == token:
        return true
    for drift in range(-TOTP_DRIFT_WINDOWS, TOTP_DRIFT_WINDOWS + 1):
        if drift == 0:
            continue
        if GenerateTOTP(secret, timestamp + drift * TOTP_STEP_SECONDS) == token:
            return true
    return false
