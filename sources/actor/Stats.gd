...
func RefreshEntityStats():
	...
	# SOM-IDLE: elemental combat (ELEMENTAL_COMBAT.md)
	current.fireDamage		= Formula.GetFireDamage(self)
	current.iceDamage		= Formula.GetIceDamage(self)
	current.lightningDamage = Formula.GetLightningDamage(self)
	current.fireResist		= Formula.GetFireResist(self)
	current.iceResist		= Formula.GetIceResist(self)
